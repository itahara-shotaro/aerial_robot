...
Eigen::MatrixXd GimbalrotorRobotModel::calcMaskedWrenchMatrixOnCoG(int gimbal_dof)
{
  ...
  const std::vector<KDL::Rotation> thrust_coords_rot = getThrustCoordRot<KDL::Rotation>();
  Eigen::MatrixXd integrated_rot = Eigen::MatrixXd::Zero(3 * rotor_num, rotor_coef * rotor_num);
  for (int i = 0; i < rotor_num; i++)
  {
    tf::Quaternion rotation;
    tf::quaternionKDLToTF(thrust_coords_rot.at(i), rotation);
    Eigen::Matrix3d cog_from_thrust;
    tf::matrixTFToEigen(tf::Matrix3x3(rotation), cog_from_thrust);
    ...
    integrated_rot.block(3 * i, rotor_coef * i, 3, rotor_coef) = cog_from_thrust * mask;
  }
